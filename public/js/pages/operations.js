...
export async function renderOperations() {
  ...
  try {
    ...
    $('#ops').outerHTML = `<div class="grid">
      ...
      <section class="card span-5"><h2>Configured resource drift</h2>${drift.length ? drift.slice(0, 20).map(item => `<div class="notice error"><strong>${escapeHtml(item.label)}</strong><br><span class="small">${escapeHtml(item.module)} · ${escapeHtml(item.resourceType)} ${escapeHtml(item.resourceId)}</span></div>`).join('') : '<div class="notice success">No missing configured channels or roles.</div>'}</section>

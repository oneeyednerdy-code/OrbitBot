...
Short-Form Video has its own D1 queue and worker lifecycle. It validates connected YouTube/TikTok/Instagram scopes, accepts a server-side uploaded MP4/MOV/WebM file through the optional R2 binding or a public HTTPS media URL, posts to the provider-specific API, tracks asynchronous TikTok/Instagram processing, and exposes now/scheduled/cancel/retry/delete controls. Social Management counts Unicode code points in the browser and repeats server-side checks for Discord, Bluesky, Threads, and the connected Mastodon instance limit.

Channel Manager edits existing categories and channels through a live preview. It validates names, category membership, category capacity, channel-specific settings, owner confirmation, automatic structural backup, queued Discord PATCH operations, per-item outcomes, and audit history without adding Gateway intents.

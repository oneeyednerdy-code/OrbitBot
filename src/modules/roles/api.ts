...
export async function rolesApi(request:Request,env:Env,guildId:string,actorId:string):Promise<Response>{
  ...
  if(request.method==='POST'){
    ...
    return body.operation==='update_panel'?updatePanel(body,env,guildId,actorId):createPanel(body,env,guildId,actorId);
  }
  if(request.method==='DELETE')return deletePanel(request,env,guildId);
  return json({error:'method_not_allowed'},405);
}

async function createPanel(body:any,env:Env,guildId:string,actorId:string):Promise<Response>{
  const name=String(body.name||'').trim().slice(0,80),channelId=String(body.channel_id||''),message=String(body.message||`**${name}**\nChoose the roles you want.`);
...
async function loadPanelForEdit(env:Env,guildId:string,id:number):Promise<Response>{
  const panel=await env.DB.prepare('SELECT * FROM role_panels WHERE id=? AND guild_id=?').bind(id,guildId).first<any>();
  if(!panel)return json({error:'panel_not_found',detail:'That role panel no longer exists.'},404);
  panel.items=(await env.DB.prepare('SELECT * FROM role_panel_items WHERE panel_id=? ORDER BY sort_order,id').bind(id).all<any>()).results;
  const config=parseConfig(panel.config_json),fallback=String(config.message||`**${panel.name}**\nChoose the roles you want.`);
  panel.message=fallback;
  panel.message_source=config.message?'saved':'fallback';
  if(panel.channel_id&&panel.message_id){
    const current=await discord(env,`/channels/${panel.channel_id}/messages/${panel.message_id}`);
    if(current.ok){const discordMessage=await current.json<any>();panel.message=String(discordMessage.content||fallback);panel.message_source='discord';}
    else panel.message_warning=current.status===404?'The original Discord message is missing. Saving will post a replacement in the same channel.':'Orbit could not read the current Discord message. Saving will use the text shown here.';
  }
  return json({panel});
}

async function updatePanel(body:any,env:Env,guildId:string,actorId:string):Promise<Response>{
  const panelId=Number(body.panel_id);
  ...
  if(!rolesResponse.ok||!memberResponse.ok||!channelsResponse.ok)return json({error:'discord_validation_failed',detail:'Orbit could not validate the server roles, bot hierarchy, and destination channel.'},502);
  const roles=await rolesResponse.json<any[]>(),botMember=await memberResponse.json<any>(),channels=await channelsResponse.json<any[]>();
  if(!channels.some(channel=>String(channel.id)===channelId&&(channel.type===0||channel.type===5)))return json({error:'invalid_channel',detail:'The panel destination is no longer a text or announcement channel.'},400);
  const botTop=Math.max(...roles.filter(role=>botMember.roles?.includes(role.id)).map(role=>Number(role.position)),0);
  const selected=new Map<string,{role_id:string;label:string;emoji:string|null}>();
  for(const raw of body.items){const roleId=String(raw.role_id||'');if(/^\d+$/.test(roleId)&&!selected.has(roleId))selected.set(roleId,{role_id:roleId,label:String(raw.label||'Role').slice(0,80),emoji:raw.emoji?String(raw.emoji).slice(0,64):null});}
  const items=[...selected.values()];
  if(items.length<1)return json({error:'no_roles_selected',detail:'Choose at least one role.'},400);
  if(items.length>10)return json({error:'too_many_roles',detail:'Role panels can contain up to 10 roles.'},400);
  for(const item of items){const role=roles.find(candidate=>String(candidate.id)===item.role_id);if(!role||role.name==='@everyone'||role.managed||Number(role.position)>=botTop)return json({error:'role_not_assignable',detail:`Orbit cannot assign ${role?.name||item.label}. Move Orbit above that role or choose another role.`},400);}

  const type=body.interaction_type==='select'?'select':'button',components=panelComponents(panelId,type,items);
  let messageId=String(panel.message_id||''),discordResponse:Response|null=null,replaced=false;
  if(messageId)discordResponse=await discord(env,`/channels/${channelId}/messages/${messageId}`,{method:'PATCH',body:JSON.stringify({content:message,components,allowed_mentions:{parse:[]}})});
  if(!messageId||discordResponse?.status===404){
    discordResponse=await discord(env,`/channels/${channelId}/messages`,{method:'POST',body:JSON.stringify({content:message,components,allowed_mentions:{parse:[]}})});
    if(discordResponse.ok){const replacement=await discordResponse.json<any>();messageId=String(replacement.id);replaced=true;}
  }
  if(!discordResponse?.ok){let detail:any={};try{detail=await discordResponse?.json<any>()}catch{}const requestId=await recordSystemError(env,guildId,'/channels/:channel/messages/:message','PATCH',discordResponse?.status||500,'role_panel_update_failed',detail);return json({error:'role_panel_update_failed',status:discordResponse?.status||500,detail:detail?.message||'Discord would not update the role panel message.',request_id:requestId},400);}
...
function panelComponents(panelId:number,type:string,items:{role_id:string;label:string;emoji:string|null}[]){return type==='select'?[{type:1,components:[{type:3,custom_id:`orbit_roles:${panelId}`,min_values:0,max_values:items.length,placeholder:'Choose your roles',options:items.map(item=>({label:item.label,value:item.role_id,...(item.emoji?{emoji:{name:item.emoji}}:{})}))}]}]:chunk(items,5).map(row=>({type:1,components:row.map(item=>({type:2,style:2,label:item.label,custom_id:`orbit_role:${panelId}:${item.role_id}`,...(item.emoji?{emoji:{name:item.emoji}}:{})}))}));}
function parseConfig(value:unknown):Record<string,any>{try{return JSON.parse(String(value||'{}'))||{}}catch{return {}}}

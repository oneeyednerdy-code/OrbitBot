  ...
  for(const platform of targets){
    ...
    if(platform==='discord'){
      ...
      if(integration?.discord_channel_id&&await isGuildMessageChannel(env,String(post.guild_id),String(integration.discord_channel_id))){
        ...
        status=response.ok?'sent':'failed';error=response.ok?null:String(response.status);transientFailure ||= response.status===429||response.status>=500;if(response.ok)externalId=(await response.json<any>()).id;
      }
    }else{
      ...
      if(integration){const response=await publishExternal(env,integration,String(post.content),`orbit-social-${id}-${platform}`);status=response.ok?'sent':'failed';externalId=response.externalId||null;error=response.error||null;transientFailure ||= Boolean(response.transient);}
    }

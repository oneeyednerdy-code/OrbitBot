...
export async function refreshDiagnostics(persist=false){
  ...
  try{
    last=await api(`/api/guilds/${state.guildId}/diagnostics`,{method:persist?'POST':'GET'});
    render(last);
  }catch(e){if(label)label.textContent=`Diagnostics unavailable · ${e.message}`;}
}
function render(data){
  const label=document.querySelector('#diagSummary');const dot=document.querySelector('#diagDot');const list=document.querySelector('#diagChecks');
...
async function submitBug(event){
  ...
  const client={user_agent:navigator.userAgent,online:navigator.onLine,errors:form.get('include_errors')?clientDiagnostics.errors.slice(-10):[],network_failures:form.get('include_network')?clientDiagnostics.networkFailures.slice(-10):[]};
  try{

...
export async function rolesApi(request: Request, env: Env, guildId: string, actorId: string): Promise<Response> {
  ...
  if (request.method === 'POST') {
    ...
    if(sent.ok){const msg=await sent.json<any>();await env.DB.prepare('UPDATE role_panels SET message_id=? WHERE id=?').bind(msg.id,panelId).run();return json({ok:true,panel_id:panelId,message_id:msg.id});}
  ...
  if(request.method==='DELETE'){
    ...
    // Deleting a panel is intentionally non-destructive to members: assigned Discord roles remain assigned.
    if(panel.message_id&&panel.channel_id){
      ...
      if(!removed.ok&&removed.status!==404){
        let detail:any={};try{detail=await removed.json<any>()}catch{}
        const requestId=await recordSystemError(env,guildId,'/channels/:channel/messages/:message','DELETE',removed.status,'role_panel_message_delete_failed',detail);
    ...
    await env.DB.batch([
      env.DB.prepare('DELETE FROM role_panel_items WHERE panel_id IN (SELECT id FROM role_panels WHERE id=? AND guild_id=?)').bind(id,guildId),
      env.DB.prepare('DELETE FROM role_panels WHERE id=? AND guild_id=?').bind(id,guildId),
    ]);

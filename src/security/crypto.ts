...
async function aesKey(secret: string): Promise<CryptoKey> {
  const digest = await crypto.subtle.digest('SHA-256', buffer(encoder.encode(secret)));
  return crypto.subtle.importKey('raw', digest, { name: 'AES-GCM' }, false, ['encrypt', 'decrypt']);
}

export async function seal(value: string, secret: string): Promise<string> {
  const iv = crypto.getRandomValues(new Uint8Array(12));
  const encrypted = await crypto.subtle.encrypt({ name: 'AES-GCM', iv: buffer(iv) }, await aesKey(secret), buffer(encoder.encode(value)));
...
export async function openSeal(value: string, secret: string): Promise<string> {
  ...
  const out = await crypto.subtle.decrypt({ name: 'AES-GCM', iv: buffer(unbase64(iv)) }, await aesKey(secret), buffer(unbase64(encrypted)));
  return new TextDecoder().decode(out);
}

export async function sha256(value: string): Promise<string> {
  const digest = new Uint8Array(await crypto.subtle.digest('SHA-256', buffer(encoder.encode(value))));
...
export async function verifyEd25519(publicKey: string, signature: string, message: string): Promise<boolean> {
  try {
    ...
    return crypto.subtle.verify('Ed25519', key, buffer(hex(signature)), buffer(encoder.encode(message)));
  } catch {
    return false;
  }
}

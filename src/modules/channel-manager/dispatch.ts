...
function channelOperationDetail(row:any,response:Response,detail:any):string{const discordCode=String(detail?.code||'');if(response.status===403&&discordCode==='50001')return `Orbit could not access ${row.name||'this channel'} while deleting it. Grant Orbit View Channel and Manage Channels in that channel or its category, then retry. Discord returned Missing Access (50001).`;if(response.status===403&&discordCode==='50013')return `Orbit is missing permission to change ${row.name||'this channel'}. Grant Orbit Manage Channels and make sure a category overwrite is not denying it. Discord returned Missing Permissions (50013).`;return detail?.message||`Discord returned HTTP ${response.status}.`}

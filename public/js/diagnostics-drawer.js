...
function render(data){
  ...
  if(label)label.textContent=data.status==='healthy'?`Core systems look good · ${data.score}%`:`${data.score}% core health · ${data.status}`;
...
function safeReport(){return {version:'0.1.0-alpha.50',generated_at:new Date().toISOString(),guild:'redacted',page:state.page,diagnostics:last,client:{user_agent:navigator.userAgent,online:navigator.onLine,errors:clientDiagnostics.errors.slice(-50),network_failures:clientDiagnostics.networkFailures.slice(-50),request_log:clientDiagnostics.requests.slice(-100)},privacy:'Verbose logs are sanitized. Tokens, secrets, cookies, authorization headers, credentials and message contents are excluded.'}}
...
function errorRow(x){return `<div class="diag-error-row"><strong>${escapeHtml(x.error_code)}</strong><span class="small">${escapeHtml(x.method)} ${escapeHtml(x.route)} · HTTP ${escapeHtml(x.status)} · ${new Date(x.created_at).toLocaleString()} · ${escapeHtml(x.request_id)}</span><code>${escapeHtml(JSON.stringify(x.detail||{}))}</code></div>`}

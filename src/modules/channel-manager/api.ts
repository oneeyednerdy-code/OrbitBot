...
async function previewDelete(env:Env,guildId:string,body:any):Promise<Response>{
  const channels=await getChannels(env,guildId),guild=await getDiscordGuild(env,guildId),dependencies=await scanDependencies(env,guildId);
  const requested=new Set<string>([...(Array.isArray(body.channel_ids)?body.channel_ids:[]),...(Array.isArray(body.category_ids)?body.category_ids:[])].map(String));
  const categories=new Set(channels.filter(c=>c.type===4&&requested.has(c.id)).map(c=>c.id));
  if(body.cascade_categories)for(const channel of channels)if(channel.parent_id&&categories.has(channel.parent_id))requested.add(channel.id);
  ...
  const targets=channels.filter(c=>requested.has(c.id));
  ...
  return json({targets:deletable,blocked,cascade_categories:Boolean(body.cascade_categories),fingerprint,confirmation_phrase:`DELETE ${deletable.length} CHANNELS`,warning:'Discord permanently deletes every selected channel and its message history. Orbit backups restore channel structure and permissions only—never deleted messages, threads, attachments, webhooks, or original channel IDs.'});
}
...
async function snapshot(env:Env,guildId:string,actorId:string,name:string,source:string,targetIds:string[]):Promise<number>{
  const channels=await getChannels(env,guildId);const data={version:1,guild_id:guildId,channels,captured_at:Date.now(),limitations:['No messages','No threads','No attachments','No webhooks','Restored channels receive new Discord IDs']};
...
async function getChannels(env:Env,guildId:string):Promise<Channel[]>{const response=await discord(env,`/guilds/${guildId}/channels`);if(!response.ok)throw new Error(`discord_channels_${response.status}`);const raw=await response.json<any[]>();return raw.filter(c=>STRUCTURAL_TYPES.has(Number(c.type))).map(c=>({id:String(c.id),name:String(c.name||'Unnamed'),type:Number(c.type),parent_id:c.parent_id?String(c.parent_id):null,position:Number(c.position||0),topic:c.topic??null,nsfw:Boolean(c.nsfw),rate_limit_per_user:Number(c.rate_limit_per_user||0),bitrate:Number(c.bitrate||0),user_limit:Number(c.user_limit||0),permission_overwrites:Array.isArray(c.permission_overwrites)?c.permission_overwrites.map((o:any)=>({id:String(o.id),type:Number(o.type),allow:String(o.allow||'0'),deny:String(o.deny||'0')})):[]})).sort((a,b)=>a.position-b.position);}

...
export async function renderEvents(){
  ...
  try{
    ...
    const permission=data.event_permission||{ok:true};
    const permissionNotice=permission.ok?'':`<div class="notice error"><strong>Orbit needs Create Events permission</strong><br><span class="small">${escapeHtml(permission.detail||'Reauthorize Orbit with the updated permission set before creating a Discord Scheduled Event.')}</span><br><a class="btn ghost" href="${escapeHtml(permission.install_url||`/oauth/install?guild_id=${encodeURIComponent(state.guildId)}`)}">Reauthorize Orbit</a></div>`;
    $('#events').outerHTML=`<div id="eventNotice">${permissionNotice}</div><div class="grid"><section class="card span-5"><h2>Create event</h2><label>Name<input id="evName"></label><label>Description<textarea id="evDesc"></textarea></label><label>Starts<input id="evStart" type="datetime-local"></label><label>Ends<input id="evEnd" type="datetime-local"></label><label>Repeat<select id="evRepeat"><option value="">Does not repeat</option><option value="daily">Daily</option><option value="weekly">Weekly</option><option value="biweekly">Every 2 weeks</option><option value="monthly">Monthly</option></select></label><label>Location<input id="evLocation" value="Discord"></label><label>RSVP panel channel<select id="evChannel">${channelOptions}</select></label><label>Optional role ping<select id="evPingRole">${roleOptions}</select></label><label class="check"><input id="evDiscord" type="checkbox" checked>Create Discord Scheduled Event</label><label class="check"><input id="evRsvp" type="checkbox" checked>Post RSVP buttons in the selected channel</label><button id="evAdd" class="btn">Create Event</button><div class="small" style="margin-top:10px">RSVP buttons consolidate event signups in Orbit. Repeating events use Discord's native recurrence and require Create Events permission.</div></section><section class="card span-7"><h2>Upcoming</h2>${events.length?events.map(x=>{const recurrence=parseRecurrence(x.recurrence_rule_json);return `<div class="notice"><strong>${escapeHtml(x.name)}</strong><br><span class="small">${new Date(x.starts_at).toLocaleString()} ${recurrence?`· Repeats ${escapeHtml(recurrence)}`:''} ${x.discord_event_id?'· Discord event created':''} ${x.event_message_id?'· RSVP panel posted':''}</span><br><span class="small">Going: ${Number(x.going_count||0)} · Maybe: ${Number(x.maybe_count||0)}</span><button class="btn ghost evDel" data-id="${x.id}">Remove</button></div>`}).join(''):'<div class="empty">No events scheduled.</div>'}</section></div>`;

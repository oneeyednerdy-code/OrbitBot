import { $, api, cancelPageRender, escapeHtml, normalizeBundle, sortGuilds, state } from './core.js';
...
import { navigationItem, renderNavigation } from './navigation.js';
...
async function refreshGuildData(){
  ...
  try{
    ...
    state.bundle=normalizeBundle(fresh);applyAdaptiveNavigation();setActiveNav();renderPage();
  }catch(error){
...
function pageAllowed(page){if(page==='onboarding')return true;const item=navigationItem(page);if(!item)return false;if(item.ownerOnly&&!state.bundle?.guild?.owner)return false;if(item.operatorOnly&&!state.me?.operator)return false;const enabled=enabledFeatures();return !item.features.length||item.features.some(feature=>enabled.has(feature));}
function applyAdaptiveNavigation(){
  ...
  document.querySelectorAll('[data-owner-only]').forEach(el=>el.classList.toggle('hidden',!state.bundle?.guild?.owner));
  document.querySelectorAll('[data-operator-only]').forEach(el=>el.classList.toggle('hidden',!state.me?.operator));
...
function setSidebarOpen(open){const sidebar=$('#sidebar'),menu=$('#menu'),scrim=$('#sidebarScrim');sidebar.classList.toggle('open',open);menu?.setAttribute('aria-expanded',String(open));scrim?.classList.toggle('hidden',!open);}
function wireNavigation(){
  document.addEventListener('click',event=>{const link=event.target.closest('[data-page]');if(!link)return;event.preventDefault();const page=link.dataset.page;if(!pageAllowed(page)||page===state.page)return;cancelPageRender();state.page=page;history.replaceState(null,'',`#${state.page}`);setActiveNav();showPageTransition(page);setSidebarOpen(false);requestAnimationFrame(()=>requestAnimationFrame(()=>{if(state.page===page)renderPage()}));});
...
async function loadServerChannelCounts(){const button=$('#loadServerChannelCounts'),status=$('#serverSearchStatus');if(!button)return;if(serverSortMode==='size'){serverSortMode='name';renderServerPicker();if(status)status.textContent='Servers sorted alphabetically.';button.textContent='Sort by size';return;}button.disabled=true;button.textContent='Loading counts…';if(status)status.textContent='Checking channel counts…';try{state.guilds=await api('/api/guilds?include_channel_counts=1',{cache:'no-store'});serverSortMode='size';renderServerPicker();const large=state.guilds.filter(g=>Number(g.channel_count||0)>=50).length;if(status)status.textContent=`Sorted by channel count · ${large} server${large===1?'':'s'} have 50+ channels.`;button.textContent='Sort alphabetically';}catch(error){if(status)status.textContent=error.payload?.detail||`Could not load channel counts (${error.message}).`;}finally{button.disabled=false;}}

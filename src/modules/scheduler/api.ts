...
export async function schedulerApi(request:Request,env:Env,guildId:string,actorId:string):Promise<Response>{
  ...
  if(request.method==='POST'){
    ...
    if(op==='create'||op==='batch'){
      ...
      let mentionableRoles=new Map<string,string>();
      if(requestedRoleIds.length){const rolesResponse=await discord(env,`/guilds/${guildId}/roles`);if(!rolesResponse.ok)return json({error:'discord_roles_unavailable'},502);const roles=await rolesResponse.json<any[]>();mentionableRoles=new Map(roles.filter(role=>role.id!==guildId&&!role.managed&&role.mentionable).map(role=>[String(role.id),String(role.name)]));const invalid=requestedRoleIds.find(id=>!mentionableRoles.has(id));if(invalid)return json({error:'role_not_mentionable',detail:'The selected role is missing, managed by an integration, or not marked mentionable in Discord.'},409);}
      for(const item of items.slice(0,100)){if(!item.channel_id||!item.content||!Number(item.scheduled_for))continue;const roleId=String(item.ping_role_id||'')||null;const contentJson={content:String(item.content),ping_role_id:roleId,ping_role_name:roleId?mentionableRoles.get(roleId)||null:null};const r=await env.DB.prepare('INSERT INTO scheduled_posts(guild_id,channel_id,content_json,timezone,scheduled_for,recurrence_rule,status,created_by,created_at,updated_at,paused,name) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)').bind(guildId,String(item.channel_id),JSON.stringify(contentJson),String(item.timezone||'UTC'),Number(item.scheduled_for),item.recurrence_rule||null,'queued',actorId,now,now,0,item.name||null).run();ids.push(Number(r.meta.last_row_id));}return json({ok:true,ids});
    }
    if(op==='action'){

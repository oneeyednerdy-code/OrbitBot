...
export async function renderEvents(){
  ...
  try{
    ...
    $('#events').outerHTML=`<div id="eventNotice"></div><div class="grid"><section class="card span-5"><h2>Create event</h2><label>Name<input id="evName"></label><label>Description<textarea id="evDesc"></textarea></label><label>Starts<input id="evStart" type="datetime-local"></label><label>Ends<input id="evEnd" type="datetime-local"></label><label>Repeat<select id="evRepeat"><option value="">Does not repeat</option><option value="daily">Daily</option><option value="weekly">Weekly</option><option value="biweekly">Every 2 weeks</option><option value="monthly">Monthly</option></select></label><label>Location<input id="evLocation" value="Discord"></label><label class="check"><input id="evDiscord" type="checkbox" checked>Create Discord Scheduled Event</label><button id="evAdd" class="btn">Create Event</button><div class="small" style="margin-top:10px">Repeating events use Discord's native recurrence and require Orbit to have Create Events permission.</div></section><section class="card span-7"><h2>Upcoming</h2>${data.events.length?data.events.map(x=>{const recurrence=parseRecurrence(x.recurrence_rule_json);return `<div class="notice"><strong>${escapeHtml(x.name)}</strong><br><span class="small">${new Date(x.starts_at).toLocaleString()} ${recurrence?`· Repeats ${escapeHtml(recurrence)}`:''} ${x.discord_event_id?'· Discord event created':''}</span><button class="btn ghost evDel" data-id="${x.id}">Remove</button></div>`}).join(''):'<div class="empty">No events scheduled.</div>'}</section></div>`;

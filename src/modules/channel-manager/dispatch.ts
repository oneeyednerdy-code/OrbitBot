...
async function failItem(env:Env,job:any,row:any,response:Response,code:string):Promise<any>{let detail:any={};try{detail=await response.clone().json<any>()}catch{}const requestId=await recordSystemError(env,job.guild_id,'channel-manager/discord',job.operation==='delete'?'DELETE':job.operation==='edit'?'PATCH':'POST',response.status,code,detail);await finishItem(env,row.id,'failed',String(detail?.code||code),requestId,null);return {item_id:row.id,name:row.name,status:response.status,discord_code:detail?.code||null,error:code,detail:channelOperationDetail(row,response,detail),request_id:requestId}}
function channelOperationDetail(row:any,response:Response,detail:any):string{const discordCode=String(detail?.code||'');if(response.status===403&&discordCode==='50001')return `Orbit could not delete ${row.name||'this channel'}. Discord returned Missing Access (50001). Refresh Channel Manager and preview again; the channel may be stale, hidden from Orbit, or no longer belong to this server. Deletion uses guild-level Manage Channels and does not require View Channel on every channel.`;if(response.status===403&&discordCode==='50013')return `Orbit is missing guild-level Manage Channels for ${row.name||'this channel'}. Update the Orbit bot role, then refresh Channel Manager and preview again. Discord returned Missing Permissions (50013).`;return detail?.message||`Discord returned HTTP ${response.status}.`}

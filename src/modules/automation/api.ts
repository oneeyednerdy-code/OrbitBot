...
export async function automationApi(request:Request,env:Env,guildId:string,actorId:string):Promise<Response>{
 ...
 if(request.method==='POST'){
  ...
  const conditions=Array.isArray(body.conditions)?body.conditions:[],actions=Array.isArray(body.actions)?body.actions:[];
  if(!String(body.name||'').trim()||body.trigger?.type!=='message_create'||actions.length<1||actions.length>10||conditions.length>10)return json({error:'invalid_automation',detail:'Choose a name, the message trigger, up to 10 conditions, and 1–10 supported actions.'},400);
  ...
  const r=await env.DB.prepare('INSERT INTO automations(guild_id,name,enabled,trigger_json,conditions_json,actions_json,created_by,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?)').bind(guildId,String(body.name).trim().slice(0,100),body.enabled===false?0:1,JSON.stringify(body.trigger),JSON.stringify(conditions),JSON.stringify(actions),actorId,now,now).run();return json({ok:true,id:Number(r.meta.last_row_id)});
 }

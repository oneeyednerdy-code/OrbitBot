# Start Here — Orbit v0.1.0-alpha.62

Orbit alpha.62 is a cumulative deployable build. You do **not** deploy the older patches one-by-one.

## Alpha.62 server-owner My Stream

Apply migration `0046_owner_stream_alerts.sql`, deploy, and open **Community Alerts**. The Discord server owner sees a private **My Stream** section where they can connect their Twitch account through OAuth, choose the Discord channel for live alerts, optionally select a Mentionable role, edit the message, and enable or delete the configuration. Community Streamers remains a separate role-gated workflow. Reconnect Twitch from My Stream if the account was authorized before alpha.62 so Orbit can identify the Twitch login needed for polling.

## Alpha.60 direct video uploads and Channel Manager editing

Apply migrations through `0046_owner_stream_alerts.sql`, deploy, and use the included R2 `orbit_storage` binding for the `orbit-storage` bucket to enable the Short-Form Video file picker. The selected MP4, MOV, or WebM streams to Orbit, is stored under a random guild-scoped key, and can be reused by the scheduled queue. The public HTTPS URL option remains available for existing workflows. Channel Manager now previews, backs up, and queues edits for existing categories and channels, including names, category membership, topics, slowmode, NSFW, bitrate, and user limits.

...
import { handleBirthdayInteraction } from '../birthdays/interactions';
...
export async function handleInteractions(request: Request, env: Env): Promise<Response> {
  ...
  const birthdayResponse = await handleBirthdayInteraction(env, interaction);
  if (birthdayResponse) return json(birthdayResponse);

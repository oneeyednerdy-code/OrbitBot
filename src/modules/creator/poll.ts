...
async function feedLatest(value:string){const url=publicHttpsUrl(value);if(!url)throw new Error('feed_url_not_public_https');const r=await fetchWithTimeout(url,{headers:{'user-agent':'OrbitBot/0.1 creator notifier'},redirect:'error'});if(!r.ok)throw new Error(`feed_${r.status}`);const xml=(await r.text()).slice(0,2_000_000);const block=match(xml,/<item\b[\s\S]*?<\/item>/i)||match(xml,/<entry\b[\s\S]*?<\/entry>/i);if(!block)return null;const title=decode(tag(block,'title')||'New post');const id=decode(tag(block,'guid')||tag(block,'id')||attr(block,'link','href')||tag(block,'link')||title);const urlOut=decode(attr(block,'link','href')||tag(block,'link')||'');return {id,title,url:urlOut};}

...
export async function renderShortVideo(){
  ...
  try{
    ...
    const uploadHelp=data.upload_enabled?`Choose an MP4, MOV, or WebM up to ${formatBytes(maxBytes)}. Orbit stores it for the queue and removes unused uploads after the retention window.`:'Direct uploads need the orbit_storage R2 binding. You can still use a public HTTPS video URL below.';
